...
function markerIdOf(variant: ArrowheadVariant): string {
  if (variant === ArrowheadVariant.ARROWHEAD_OPEN) { return 'ah-open'; }
  else if (variant === ArrowheadVariant.ARROWHEAD_DIAMOND) { return 'ah-diamond'; }
  else if (variant === ArrowheadVariant.ARROWHEAD_CIRCLE) { return 'ah-circle'; }
  else if (variant === ArrowheadVariant.ARROWHEAD_TEE) { return 'ah-tee'; }
  else { return 'ah-filled'; }
}

function markerElementFor(id: string): string {
  if (id === 'ah-open') {
    return `<marker id="ah-open" markerWidth="10" markerHeight="7" refX="10" refY="3.50" orient="auto"><polyline points="0,0 10,3.50 0,7" fill="none" stroke="context-stroke" stroke-width="1"/></marker>`;
  } else if (id === 'ah-diamond') {
    return `<marker id="ah-diamond" markerWidth="12" markerHeight="8" refX="10" refY="4" orient="auto"><polygon points="0,4 5,0 10,4 5,8" fill="context-stroke"/></marker>`;
  } else if (id === 'ah-circle') {
    return `<marker id="ah-circle" markerWidth="8" markerHeight="8" refX="4" refY="4" orient="auto"><circle cx="4" cy="4" r="3" fill="context-stroke"/></marker>`;
  } else if (id === 'ah-tee') {
    return `<marker id="ah-tee" markerWidth="4" markerHeight="8" refX="0" refY="4" orient="auto"><line x1="0" y1="0" x2="0" y2="8" stroke="context-stroke" stroke-width="1"/></marker>`;
  } else {
    return `<marker id="ah-filled" markerWidth="10" markerHeight="7" refX="10" refY="3.50" orient="auto"><polygon points="0,0 10,3.50 0,7" fill="context-stroke"/></marker>`;
  }
}
...
export function arrowMarkers(variants: ArrowheadVariant[]): string {
  ...
  for (const variant of variants) {
    if (variant === ArrowheadVariant.ARROWHEAD_NONE) { continue; }
    const id: string = markerIdOf(variant);
    if (!seen.has(id)) {
      seen.add(id);
      markers.push(markerElementFor(id));
    }
  }
