...
function edgePath(sections: EdgeSection[]): string {
...
function textElement(
  label: Array<{ locale: string; source: string }>,
...
function arrowMarkers(variants: Set<ArrowheadVariant>): string {
...
function viewBox(diagram: LaidOutDiagram): string {
...
function renderEdge(edge: LaidOutEdge): string {
  ...
  const sc = resolveStrokeColor(edge.connection.stroke);
  ...
  const markerStart = isRealVariant(startVariant) ? ` marker-start="url(#${arrowheadId(startVariant)})"` : '';
  const markerEnd = isRealVariant(endVariant) ? ` marker-end="url(#${arrowheadId(endVariant)})"` : '';
  const pathEl = `<path d="${d}" fill="none" stroke="${sc}" stroke-width="${r(sw)}"${markerStart}${markerEnd} />`;
