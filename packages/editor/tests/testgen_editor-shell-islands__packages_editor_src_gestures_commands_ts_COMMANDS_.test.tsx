// Generated by `archegraph testgen`, then repaired by hand. Regenerating
// discards the repairs: the two zoom-fit cases below were rewritten when
// zoom-fit started leaving the island columns clear.
...
import { CANVAS_INSETS } from '../src/shell/island_frame';

describe('testgen_gestures__COMMANDS', () => {
    type SpyViewport = { zoom: number; panX: number; panY: number };

    /** Width zoom-fit actually has to work with, once the columns are clear. */
    const clearWidth = (width: number): number =>
      width - Math.min(CANVAS_INSETS.left + CANVAS_INSETS.right, width / 2);
    ...
    test('zoom_fit_valid_geometry', () => {
        ...
        expect(context.viewport.zoom).toBeCloseTo(Math.min((clearWidth(value.rect.width) - 80) / 200, (value.rect.height - 80) / 100), 12);
            })
        );
    });
    ...
    test('zoom_fit_content_bounds_source', () => {
        ...
        commandFor('zoom-fit').run(context);
        expect(context.viewport.zoom).toBeCloseTo(Math.min((clearWidth(value.rect.width) - 80) / 200, (value.rect.height - 80) / 200), 12);
