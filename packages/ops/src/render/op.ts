...
export function deriveOutPath(diagramPath: string): string {
  const lastDot = diagramPath.lastIndexOf('.');
  const base = lastDot >= 0 ? diagramPath.slice(0, lastDot) : diagramPath;
  return base + '.svg';
}
...
export const renderOp: Operation<RenderParams, RenderOutput> = {
  ...
  async execute(params: RenderParams, ctx: OpContext): Promise<RenderOutput> {
    const diagramText = await fs.readFile(path.resolve(ctx.projectRoot, params.diagram), 'utf8');
    ...
    if (params.style !== undefined) {
      const styleText = await fs.readFile(path.resolve(ctx.projectRoot, params.style), 'utf8');
    ...
    if (params.theme !== undefined) {
      const themeText = await fs.readFile(path.resolve(ctx.projectRoot, params.theme), 'utf8');
    ...
    const outPath = path.resolve(ctx.projectRoot, params.out ?? deriveOutPath(params.diagram));
